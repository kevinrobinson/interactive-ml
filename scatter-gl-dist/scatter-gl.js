    ...
    ScatterGL.prototype.generatePointColorArray = function (dataset) {
        ...
        }
        {
            ...
            if (selectedPointCount > 0) {
                var c = color_1.parseColor(unselectedColor);
                for (var i = 0; i < n; ++i) {
                    colors[dst++] = c.r;
                    colors[dst++] = c.g;
                    colors[dst++] = c.b;
                ...
                if (pointColorer) {
                    for (var i = 0; i < n; ++i) {
                        var c = color_1.parseColor(pointColorer(i) || noSelectionColor);
                        colors[dst++] = c.r;
                        colors[dst++] = c.g;
                        colors[dst++] = c.b;
                        colors[dst++] = c.opacity;
                    ...
                    var c = color_1.parseColor(noSelectionColor);
                    for (var i = 0; i < n; ++i) {
                        colors[dst++] = c.r;
                        colors[dst++] = c.g;
                        colors[dst++] = c.b;
        ...
        {
            var n = selectedPointCount;
            var c = color_1.parseColor(colorSelected);
            for (var i = 0; i < n; ++i) {
                var dst = selectedPointIndices[i] * constants_1.RGBA_NUM_ELEMENTS;
                colors[dst++] = c.r;
                colors[dst++] = c.g;
                colors[dst++] = c.b;
                colors[dst++] = c.opacity;
        ...
        if (hoverPointIndex != null) {
            var c = color_1.parseColor(colorHover);
            var dst = hoverPointIndex * constants_1.RGBA_NUM_ELEMENTS;
            colors[dst++] = c.r;
            colors[dst++] = c.g;
            colors[dst++] = c.b;
            colors[dst++] = c.opacity;
        ...
        for (var i = 0; i < this.sequences.length; i++) {
            ...
            if (pointColorer) {
                for (var j = 0; j < sequence.indices.length - 1; j++) {
                    var c1 = color_1.parseColor(pointColorer(sequence.indices[j]));
                    var c2 = color_1.parseColor(pointColorer(sequence.indices[j + 1]));
...
var ScatterPlot = (function () {
    ...
    ScatterPlot.prototype.makeOrbitControls = function (camera, cameraIs3D) {
        ...
        var occ = new three_orbitcontrols_ts_1.OrbitControls(camera, this.renderer.domElement);
        ...
        if (cameraIs3D) {
            ...
            occ.mouseButtons.ORBIT = THREE.MOUSE.RIGHT;
...
var ScatterPlotVisualizerSprites = (function () {
    ...
    ScatterPlotVisualizerSprites.prototype.createGeometry = function (pointCount) {
        ...
        geometry.addAttribute('color', new THREE.BufferAttribute(new Float32Array([]), constants_1.RGBA_NUM_ELEMENTS));
